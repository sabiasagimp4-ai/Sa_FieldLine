...
#define SA_WALK(SGN, N, HSTEP, MODE, OUT_RGB, OUT_LIC) { \
...
D2D_PS_ENTRY(main)
{
    ...
    float amp = D2DSampleInputAtPosition(2, saClampToRect(p * FIELD_SCALE, FIELD_RECT)).r;
    ...
    if (GLOW > 1e-4)
    {
        ...
        float unusedF = 0.0;
        float unusedB = 0.0;
        SA_WALK(-1.0, gn, GLOW_HSTEP, 0, gf, unusedF)
        SA_WALK( 1.0, gn, GLOW_HSTEP, 0, gb, unusedB)
    ...
    if (LINE_DRAW > 1e-4)
    {
        ...
        float lf = 0.0;
        float lb = 0.0;
        SA_WALK(-1.0, ln, LINE_HSTEP, 1, unusedC, lf)
        SA_WALK( 1.0, ln, LINE_HSTEP, 1, unusedD, lb)
        float lic = 0.5 * (lf + lb);
